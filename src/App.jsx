...
const apiBaseUrl = "https://educacao-online-api.onrender.com";
...
function PainelInstrutor({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "email", "telefone", "especialidade", "bio", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "email", "telefone", "especialidade", "bio"].forEach(k => { f[k] = item[k] ?? ""; });
...
function PainelCurso({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["titulo", "descricao", "preco", "cargaHoraria", "nivel", "publicado", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["titulo", "descricao", "preco", "cargaHoraria", "nivel", "publicado"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>publicado</b><span>{item["publicado"] != null ? (item["publicado"] ? "Sim" : "Não") : "—"}</span></div>
      ...
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["publicado"] === true || form["publicado"] === "true"} onChange={e => setForm({...form, publicado: e.target.checked})} />
        publicado
      </label>
...
function PainelMatricula({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["alunoNome", "alunoEmail", "cursoId", "dataMatricula", "dataVencimento", "status", "valorPago", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["alunoNome", "alunoEmail", "cursoId", "dataMatricula", "dataVencimento", "status", "valorPago"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["alunoNome"] ?? "Matricula")}</div>
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              ...
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/matricula" + item.id + "/320/200"} alt="Matricula" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🗂️</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["alunoNome"] ?? "Matricula")}</div>
            ...
            <div className="item-field"><b>dataMatricula</b><span>{item["dataMatricula"] ? new Date(item["dataMatricula"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
      ...
      <label className="field-label">dataMatricula</label>
      <input type="datetime-local" value={form["dataMatricula"] ?? ""} onChange={e => setForm({...form, dataMatricula: e.target.value})} />
...
function PainelAula({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["cursoId", "titulo", "descricao", "duracao", "ordem", "videoUrl", "material", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["cursoId", "titulo", "descricao", "duracao", "ordem", "videoUrl", "material"].forEach(k => { f[k] = item[k] ?? ""; });
    ["curso"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["titulo"] ?? "Aula")}</div>
            ...
            <div className="item-field"><b>videoUrl</b><span>{item["videoUrl"] ?? "—"}</span></div>
            <div className="item-field"><b>material</b><span>{item["material"] ?? "—"}</span></div>
            <div className="item-field"><b>curso</b><span>{item.cursoId ? ((cursoList.find(o => o.id === item.cursoId) || {}).titulo ?? ("#" + item.cursoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
      ...
      <label className="field-label">videoUrl</label>
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Educacao Online<span className="dot">.</span></div>
